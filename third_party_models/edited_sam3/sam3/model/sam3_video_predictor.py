...
class Sam3VideoPredictor:
    ...
    def init_state(self, resource_path, numpy_img_list):
        inference_state = self.model.init_state(
            resource_path=resource_path,
            async_loading_frames=self.async_loading_frames,
            video_loader_type=self.video_loader_type,
    ...
    def add_prompt(
        ...
        inference_state = self.inference_or_sesh(inference_state, session_id)
    ...
    def inference_or_sesh(self, inference_state, session_id):
        if not inference_state:
            session = self._get_session(session_id)
        else:
            session = {'state':None}
        inference_state = session["state"] if not inference_state else inference_state
        return inference_state
    ...
    def reset_session(self, session_id, inference_state = None):
