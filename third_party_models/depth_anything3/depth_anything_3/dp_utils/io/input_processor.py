...
from depth_anything_3.dp_utils.logger import logger
from depth_anything_3.dp_utils.parallel_utils import parallel_execution
...
class InputProcessor:
    ...
    def _process_one(
        ...
        *,
        ...
        np_img = self._load_image(img)
        orig_h, orig_w, C  = np_img.shape 

        # Boundary resize
        np_img = self._resize_image(np_img, process_res, process_res_method)
        h,w, c = np_img.shape
        ...
        if process_res_method.endswith("resize"):
            np_img = self._make_divisible_by_resize(np_img, self.PATCH_SIZE)
            new_h, new_w, C = np_img.shape 
            intrinsic = self._resize_ixt(intrinsic, w, h, new_w, new_h)
            w, h = new_w, new_h
        elif process_res_method.endswith("crop"):
            np_img = self._make_divisible_by_crop(np_img, self.PATCH_SIZE)
            new_h, new_w, C = np_img.shape 
        ...
        img_tensor = self._normalize_image(np_img)
    ...
    def _normalize_image(self, img: np.array) -> torch.Tensor:
    ...
    def _make_divisible_by_resize(self, img: np.array, patch: int) -> Image.Image:
        """
