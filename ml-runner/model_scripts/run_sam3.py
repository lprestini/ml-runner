...
class runSAM3(object):
    ...
    def __init__(self, 
        ...
        self.predictor = build_sam3_video_predictor_only(checkpoint_path = self.sam3_checkpoint)
    ...
    def flip_boxes(self,box, H, W):
        ...
        box = [box[0]/ self.W,  (H-box[3]-1) /self.H, box[2]/self.W, ( H-box[3]-1 + (box[3]-box[1])) / self.H,]
        return box
    ...
    def run(self):
        ...
        if not self.inference_state:
            ...
            self.inference_state = self.predictor.init_state(resource_path=self.video_dir, numpy_img_list = self.numpy_img_list)
        ...
        for idx, b in enumerate(self.boxes_filt):
            ...
            for inference_dict in self.predictor.propagate_in_video(inference_state=self.inference_state, start_frame_idx=self.ann_frame_idx, max_frame_num_to_track=total_steps, propagation_direction='forward', session_id=0):
            ...
            if self.ann_frame_idx != 0:
                ...
                for inference_dict in self.predictor.propagate_in_video(inference_state=self.inference_state, start_frame_idx=self.ann_frame_idx, max_frame_num_to_track=total_steps, propagation_direction='backward', session_id=0):
                    out_frame_idx = inference_dict['frame_index']
                    out_obj_ids = inference_dict['outputs']['out_obj_ids'] 
                    out_mask_logits = inference_dict['outputs']['out_binary_masks']
