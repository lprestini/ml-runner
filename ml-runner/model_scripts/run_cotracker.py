...
class run_cotracker(object):
    def __init__(
        ...
        self.predictor = CoTrackerOnlinePredictor(
            checkpoint=self.pretrain_path, window_len=16, v2=False
        ).to("cuda")
    ...
    def make_grid_given_crop(self, box):
        ...
        grid_height = box[-1]
        if self.grid_size == 1:
            query = torch.tensor([grid_height / 2, grid_width / 2], device="cuda")[
                None, None
            ]
        else:
            y_range = torch.linspace(int(box[1]), int(box[-1]), int(self.grid_size))
            x_range = torch.linspace(
                int(box[0]) - 4, int(box[-2]) + 4, int(self.grid_size)
            )
    ...
    def build_tracker(
        ...
        for idx, track_idx in enumerate(tracks):
            ...
            base_format = (
                base_format.replace("REF_FRAME", str(self.first_frame_sequence))
                .replace("TRACK_NUM", f"{idx + 1}")
                .replace("COORDS_X", coord_x)
                .replace("COORDS_Y", coord_y)
            )
    ...
    def track(self, original_img_list, queries=None):
        ...
        # Track forward
        fwd_frames_padded, pad_fwd_front, pad_fwd_back = self.pad_tensor_to_multiple16(
            original_img_list, direction="tail"
        )
        fwd_frames_padded = self.prep_images(fwd_frames_padded)
        self.predictor(
            video_chunk=fwd_frames_padded,
            is_first_step=True,
        ...
        for ind in range(
            0, fwd_frames_padded.shape[1] - self.predictor.step, self.predictor.step
        ):
            ...
            write_stats_file(
                self.render_dir, [], self.uuid, "0%", tracking_progress, False
            )
        ...
        if not stop:
            ...
            if self.ann_frame_idx != 0:
                ...
                bwd_frames_padded, pad_bwd_front, pad_bwd_back = (
                    self.pad_tensor_to_multiple16(original_img_list, direction="head")
                )
                bwd_frames_padded = torch.flip(
                    self.prep_images(bwd_frames_padded), dims=[0, 1]
                )
                self.predictor(
                    video_chunk=bwd_frames_padded,
                    is_first_step=True,
                    grid_size=self.grid_size,
                    add_support_grid=True,
                    queries=queries,
                ...
                for ind in range(
                    0,
                    bwd_frames_padded.shape[1] - self.predictor.step,
                    self.predictor.step,
                ):
                    chunked = bwd_frames_padded[
                        :, ind : ind + self.predictor.step * 2, :, :
                    ]
                    bwd_pred_tracks, bwd_pred_visibility = self.predictor(
                        video_chunk=chunked
                    )
                    tracking_progress = calc_progress(
                        1, 0, (ind - self.ann_frame_idx) + 1, B
                    )
                    write_stats_file(
                        self.render_dir, [], self.uuid, "0%", tracking_progress, False
                    )
            ...
            if isinstance(queries, torch.Tensor):
                full_tracks = full_tracks[
                    :, :, : int((self.grid_size * self.grid_size)), :
                ]
        return full_tracks
    ...
    def run(self):
        ...
        if not stop:
            ...
            # Convert to tensor
            original_img_list = torch.concat(
                [
                    torch.unsqueeze(torch.from_numpy(i), dim=0)
                    for i in self.numpy_img_list
                ],
                dim=0,
            )
            ...
            if self.H > 1080 or self.W > 1920:
                ...
                original_img_list = F.interpolate(
                    original_img_list.permute(0, -1, 1, 2),
                    size=(new_h, new_w),
                    mode="bilinear",
                    align_corners=True,
                ).permute(0, 2, 3, 1)
