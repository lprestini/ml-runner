...
def load_imgs_to_numpy(frame_to_load, clip=True, rgb_only=True, to_srgb=True):
    images = []
    for idx, i in tqdm(
        enumerate(frame_to_load), total=len(frame_to_load), desc="Loading frames"
    ):
        frame = i.replace("\\", "/")
        if ".png" in i:
...
def load_mov_to_numpy(path_to_sequence, shot_name, last_frame, first_frame):
    ...
    for f in tqdm(
        enumerate(list(range(first_frame, last_frame))),
        total=len(list(range(first_frame, last_frame))),
    ):
...
def get_frame_list(path_to_sequence, shot_name, delimiter):
    ...
    frame_names = [
        p
        for p in os.listdir(path_to_sequence)
        if os.path.splitext(shot_name)[0].split(delimiter)[0] in p
        and p.endswith(".png")
        or p.endswith(".exr")
    ]
